...
const RelatedWorkers = ({services,workersId}) => {

    const {workers} = useContext(AppContext)
    const Navigate = useNavigate()

    const [relwok,setRelWok] = useState([])

    useEffect(()=>{
   if(workers.length > 0 && services) {
    const workersData= workers.filter((work) => work.services === services && work._id !== workersId)
